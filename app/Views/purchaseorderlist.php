...
<?= $this->section('main') ?>
            ...
            <?php  $empty ="";
                if(!empty($items)){
                    if (!empty($items[$purchase['clientid']]['purdet'])){ ?>
                                    ...
                                    <div class="uk-card-header">
                                        <div class="uk-grid-small uk-flex-middle" uk-grid>
                                            <div class="uk-width-expand">
                                                    <h3 class="uk-card-title uk-margin-remove-bottom"><?=$purchase['name']?></h3>
                                            </div>
                                            <div class="uk-width-auto">
                                                <button id="openorder<?=$purchase['clientid']?>" class="uk-icon-button" uk-icon="chevron-up" uk-toggle="target: #body<?=$purchase['clientid']?>" hidden></button>
                                                <button id="closeorder<?=$purchase['clientid']?>" class="uk-icon-button" uk-icon="chevron-down" uk-toggle="target: #body<?=$purchase['clientid']?>"></button>
                                            </div>
                                        </div>
                                    </div>
                                    <div id="body<?=$purchase['clientid']?>" class="uk-card-body uk-overflow-auto" hidden>
                                        <table class="uk-table uk-table-divider">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="uk-text-center uk-text-nowrap" id="qty">Jumlah Pesanan</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php foreach ($items[$purchase['clientid']]['purdet'] as $item){ ?>
                                                <tr>
                                                    <td><?=$item['name']?></td>
                                        ...
                                        <?php if(!$ismobile){?>
                                            <div class="uk-text-right" uk-margin>
                                                <button class="uk-button uk-button-primary" href="#modal-confirm<?=$purchase['clientid']?>" uk-toggle>Konfirmasi</button>
                                                <a id="deletepesanan<?=$purchase['clientid']?>" class="uk-button uk-button-danger">Hapus</a>
                                            </div>
                                        <?php }else{ ?>
                                            <button class="uk-button uk-button-primary" href="#modal-confirm<?=$purchase['clientid']?>" uk-toggle>Konfirmasi</button>
                                            <a id="deletepesanan<?=$purchase['clientid']?>" class="uk-button uk-button-danger">Hapus</a>
                        ...
                        <script>
                            document.getElementById('openorder<?=$purchase['clientid']?>').addEventListener('click', opentoggle<?=$purchase['clientid']?>);
                            document.getElementById('closeorder<?=$purchase['clientid']?>').addEventListener('click', closetoggle<?=$purchase['clientid']?>);

                            function opentoggle<?=$purchase['clientid']?>() {
                                document.getElementById('openorder<?=$purchase['clientid']?>').setAttribute('hidden', '');
                                document.getElementById('closeorder<?=$purchase['clientid']?>').removeAttribute('hidden');
                            };

                            function closetoggle<?=$purchase['clientid']?>() {
                                document.getElementById('openorder<?=$purchase['clientid']?>').removeAttribute('hidden');
                                document.getElementById('closeorder<?=$purchase['clientid']?>').setAttribute('hidden', '');
                            };

                            // Delete
                            $('#deletepesanan<?=$purchase['clientid']?>').click(function() {
                                ...
                                if (confirm(text) == true) {
                                    $.ajax({
                                        ...
                                        success: function() {
                                            ...
                                            $('#order<?=$purchase['clientid']?>').remove();
    ...
    <?php if(!empty($items)) { ?>
        <?php foreach($purchases as $purchase){ ?>
            <div id="modal-confirm<?=$purchase['clientid'] ?>" uk-modal>
                <div class="uk-modal-dialog">
                    <button class="uk-modal-close-default" type="button" uk-close></button>

                    <div class="uk-modal-header" style="background-color: #39f;">
                        <h5 class="uk-card-title uk-light"> Konfirmasi Pesanan <br> RS <?=$purchase['name'] ?></h5>
                    </div>

                    <form action="pesanmasuk/confirm/<?=$purchase['clientid']?>" method="post">
                        <div class="uk-modal-body" uk-overflow-auto>
                            <fieldset class="uk-fieldset">
                                ...
                                <table class="uk-table uk-table-divider">
                                    ...
                                    <tbody>
                                    <?php if (!empty($items[$purchase['clientid']]['purdet'])){ ?>
                                        <?php foreach ($items[$purchase['clientid']]['purdet'] as $item){ ?>
                                        <tr>
                                            <td><?=$item['name']?></td>
                                            <td class="uk-text-center"><?=$item['denomination']?></td>
